<?php
    ...
    $sql = "SELECT f.nameProvider, p.idPay, DATE_FORMAT(p.datePay, '%d/%m/%Y') as vencimento, p.valuePay, p.status FROM AGUAVIVA_PAY p, AGUAVIVA_PROVIDER f WHERE p.AGUAVIVA_PROVIDER_idProvider = f.idProvider AND 1=1 ";
    ...
    if( !empty( $filtro ) ){
        ...
        $sql .= " AND (idPay LIKE '$filtro%' ";
        $sql .= " OR nameProvider LIKE '$filtro%') ";
    }
